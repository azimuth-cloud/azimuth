    ...
    def _transform(self, instance, value):
        if value in self.choices:
            ...
            raise ImproperlyConfigured(
                f"{instance.name}.{self.name} must be one of {choices}"
            )
            ...
            authenticators.append(
                {
                    ...
                    "AUTHENTICATOR": {
                        "FACTORY": "azimuth_auth.authenticator.openstack.ApplicationCredentialAuthenticator",  # noqa: E501
                        "PARAMS": {
                ...
                authenticators.append(
                    {
                        "NAME": "federated",
                        "AUTHENTICATOR": {
                            "FACTORY": "azimuth_auth.authenticator.openstack.FederatedAuthenticator",  # noqa: E501
                            "PARAMS": {
                                "AUTH_URL": instance.OPENSTACK.AUTH_URL,
                ...
                authenticators.append(
                    {
                        "NAME": "password",
                        "AUTHENTICATOR": {
                            "FACTORY": "azimuth_auth.authenticator.openstack.PasswordAuthenticator",  # noqa: E501
                            "PARAMS": {
                                "AUTH_URL": instance.OPENSTACK.AUTH_URL,
                                "DOMAINS": [
                                    {
...
class AzimuthAuthSettings(SettingsObject):
    ...
    #: This cookie does not have an expiry date, so the selection persists beyond the
    #: session
    AUTHENTICATOR_COOKIE_NAME = Setting(default="azimuth-authenticator")

    #: For the bearer token middleware, this is the name of the header that the token
    #: will be in
    BEARER_TOKEN_HEADER = Setting(default="HTTP_AUTHORIZATION")
    #: For the bearer token middleware, this is the prefix that will be present and
    #: needs to be stripped
    BEARER_TOKEN_PREFIX = Setting(default="Bearer")

    #: For the session middleware, this is the name of the session key in which the
    #: token is stored
    ...
    MESSAGES = MergedDictSetting(
        {
            ...
            "logout_successful": (
                {"message": "You have been signed out.", "level": "success"}
            ),
        }
    )
