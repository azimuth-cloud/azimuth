        ...
        try:
            ...
            if exc.response.status_code == 400:
                ...
                raise errors.CommunicationError(
                    "Error contacting OIDC issuer."
                ) from exc
        except httpx.HTTPError as exc:
            raise errors.CommunicationError(
                "Could not connect to OIDC issuer."
            ) from exc
...
class Auth(httpx.Auth):
    ...
    def _parse_token(self, token):
        """
        Parse the given token and return an access token and refresh token,
        if available.

        The given token is either a raw access token or a base64-encoded JSON blob
        containing the full token data, as returned by the OIDC authenticator.
        """
        ...
        try:
            ...
            return token, None
        # If there is no access token in the decoded data, assume the original token is
        # the access token but just happens to be a base64-encoded JSON string :shrugs:
...
class Session(base.Session):
    ...
    In this implementation we do not use the OIDC ID token to identify the user.
    Instead, we use the OAuth2 access token to query the OIDC userinfo endpoint on every
    request.

    This decision was made, at the expense of more requests to the IDP, for a number of
    reasons:

      * It means we do not have to deal with parsing JWTs and all the things that come
        with that, e.g. fetching and caching JWKS keys and knowing when they have been
        rotated.
      * We don't have to decide how long we are happy working with stale user
        information due to ID token caching. Changes to user information at the IDP, and
        in particular group memberships, are reflected immediately.
    """
    ...
    def user(self):
        ...
        try:
            ...
            # If the user does not have the required claims, log the error and deny
            # access
            # The sub claim should ALWAYS be present, so use that to identify the user
            logger.exception(
                "required claim missing for user '%s'", self._userinfo["sub"]
            )
            raise errors.PermissionDeniedError("Permission denied.")
    ...
    def tenancies(self):
        ...
        #
        # We decide if the user is permitted to access the namespace by checking if they
        # have the group specified in the OIDC group annotation, where the absence of
        # the annotation means no OIDC groups have access
        # We use an annotation rather than a label because group names may contain
        # characters that are not valid in a label value,
        # e.g. Keycloak groups contain '/'
        #####
        user = self.user()
        ...
        # We do this to avoid non-deterministic behaviour if multiple namespaces have
        # the same ID
        namespaces_by_id = {}
        ...
        for ns in itertools.chain(
            ...
            # If we have already seen the namespace, that means it has both labels so
            # skip it
            ...
            # Use the value of the label as the ID, with the new-style label taking
            # precedence
            ...
            # If the namespace has both labels and the values are different,
            # emit a warning
            if tenancy_id_legacy and tenancy_id != tenancy_id_legacy:
                logger.warning(
                    "namespace '%s' has new-style and legacy tenancy ID labels with different values",  # noqa: E501
                    ns_name,
                )
        ...
        for tenancy_id, namespaces in namespaces_by_id.items():
            if len(namespaces) != 1:
                logger.error(
                    "tenancy ID '%s' appears on multiple namespaces", tenancy_id
                )
                continue
            ...
            tenancy_name = annotations.get(
                TENANCY_NAME_ANNOTATION, ns_name.removeprefix("az-")
            )
    ...
    def _iter_namespaces(self, tenancy_id):
        ...
        for namespace in eknamespaces.list(labels={TENANCY_ID_LABEL: tenancy_id}):
            ...
            yield namespace
        for namespace in eknamespaces.list(
            labels={TENANCY_ID_LABEL_LEGACY: tenancy_id}
        ):
    ...
    def credential(self, tenancy_id, provider):
        ...
        #
        # If no secret exists with the correct label for the provider, then we are
        # unable to supply a credential for that provider and we return None
        #####
        user = self.user()
        logger.info(
            "[%s] locating namespace for tenant ID '%s'", user.username, tenancy_id
        )
        ...
        if len(namespaces) > 1:
            ...
            logger.warning(
                "[%s] no namespace for tenant ID '%s'", user.username, tenancy_id
            )
            return None
        else:
        ...
        secrets = list(
            self._ekclient.api("v1")
            .resource("secrets")
            .list(
                labels={CLOUD_CREDENTIAL_PROVIDER_LABEL: provider}, namespace=namespace
            )
        )
        if len(secrets) > 1:
            logger.error(
        ...
        # This is to avoid non-deterministic behaviour when returning the first key
