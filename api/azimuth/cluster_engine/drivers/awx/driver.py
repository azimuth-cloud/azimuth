...
import rackit
from cluster_engine import dto, errors
...
class Driver(base.Driver):
    ...
    def __init__(
        ...
        self._connection = api.Connection(
            url.rstrip("/"), username, password, verify_ssl
        )
    ...
    def _get_team(self, ctx: dto.Context):
        ...
        try:
            ...
            # If we are not, then not finding a team for the context is a permissions
            # error
            if self._create_teams:
                self._log(
                    "No AWX team found - it will be created when required", ctx=ctx
                )
            else:
    ...
    def _get_or_create_team(self, ctx: dto.Context):
        ...
        if not team:
            ...
            team = self._connection.teams.create(
                name=ctx.tenancy.name, organization=self._organisation.id
            )
            # Create the allow all permission if required
            # This is represented in AWX as holding the execute role for the entire
            # organisation
            if self._create_team_allow_all_permission:
                ...
                execute_role = next(
                    ...
                    if (
                        role.name.lower() == "execute"
                        and role.summary_fields.get("resource_type") == "organization"
                        and role.summary_fields.get("resource_id")
                        == self._organisation.id
                    )
                )
    ...
    def _get_permitted_job_templates(self, ctx: dto.Context):
        ...
        # If the team has the execute permission for the organisation, it is permitted
        # to access all the templates
        allow_all = any(
            (
                role.name.lower() == "execute"
                and role.summary_fields["resource_type"] == "organization"
                and role.summary_fields["resource_id"] == self._organisation.id
            )
            for role in roles
        )
        if allow_all:
            ...
            return (True, set())
        # Otherwise, the team may have been granted the execute permission on specific
        # templates
        permitted = {
    ...
    def cluster_types(self, ctx: dto.Context) -> t.Iterable[dto.ClusterType]:
        ...
        """
        # Get the names of the job templates that the team has been granted execute
        # access to
    ...
    def _get_permitted_inventories(self, team: api.Team, ctx: dto.Context):
        ...
        permitted = {
            ...
            if (
                ...
                and role.summary_fields["resource_type"] == "inventory"
            )
        }
    ...
    def _from_inventory(self, inventory: api.Inventory, ctx: dto.Context):  # noqa: C901
        ...
        try:
            ...
            if latest.status == "successful":
                ...
                # Use a combination of the task and the role that it comes from
                # (if present)
        ...
        # The patched time comes from the last successful job with
        # cluster_upgrade_system_packages = True
        job = latest
        outputs = {}
        updated = None
        patched = None
        # If we haven't found the update or patch time, traverse the rest of the jobs
        # until we find them
        while job:
            if job.status == "successful":
                ...
                if not updated:
                    ...
                    # If the last task is a debug action for the "outputs" variable,
                    # then that value is used as the outputs
                    event = next(
                ...
                if json.loads(job.extra_vars).get(
    ...
    def _create_credential(self, ctx: dto.Context):
        ...
        try:
            ...
            self._log("Finding credential type '%s'", credential_type_name, ctx=ctx)
            credential_type = self._connection.credential_types.find_by_name(
    ...
    def _run_inventory(
        ...
        """
        Utility method to update inventory variables and run a job with the given
        credential.
        """
    ...
    def create_cluster(
        ...
        template_inventory = self._connection.inventories.find_by_name(
        ...
        if inventory:
            ...
            try:
                ...
                while remaining > 0:
                    try:
                        inventory = self._connection.inventories.get(
                            inventory.id, force=True
                        )
                    except rackit.NotFound:
                        break
                    else:
        ...
        admin_role = next(
            ...
            if (
                role.name.lower() == "admin"
                and role.summary_fields.get("resource_type") == "inventory"
                and role.summary_fields.get("resource_id") == inventory.id
            )
        )
