...
def build_validator(
    ...
    A validator function takes the parameters to validate as it's only argument and
    returns the validated parameters on success. On failure, a `ValidationError` is
    raised.
    """
    spec = {}
    for param in parameter_spec:
        ...
        spec[key] = v.All(
            kind_constraint(
                cloud_session, cluster_manager, param.kind, param.options, prev_value
            ),
...
def list_constraint(cloud_session, cluster_manager, options, prev_value):
    ...
    if "item" in options:
        ...
        def validate_items(value):
            # The validator to use for each element depends on whether there is an
...
def boolean_constraint(options, prev_value, **kwargs):
    ...
    return v.All(
        v.Any(
            # Don't use "bool" because that would result in a coercion which we don't
            # want
            v.In([True, False]),
...
def cloud_size_constraint(cloud_session, options, **kwargs):
    ...
    def min_ephemeral_disk(size):
        if (
            "min_ephemeral_disk" in options
            and size.ephemeral_disk < options["min_ephemeral_disk"]
        ):
    ...
    def has_properties(size):
        for prop in options.get("has_properties", []):
            ...
            if (
                "value" in prop
                and str(prop["value"]) != size.additional_properties[prop_name]
            ):
    ...
    return v.All(
        v.Coerce(str),
        convert_not_found(lambda v: cloud_session.find_size(v), "Not a valid size."),
        min_cpus,
        min_ram,
        min_disk,
        min_ephemeral_disk,
...
def cloud_machine_constraint(cloud_session, **kwargs):
    return v.All(
        v.Coerce(str),
        convert_not_found(
            lambda v: cloud_session.find_machine(v), "Not a valid machine."
        ),
        lambda m: m.id,
    )
...
def cloud_ip_constraint(cloud_session, prev_value, **kwargs):
    ...
    def ip_available(ip):
        if prev_value == ip.id or ip.available:
            return ip
        else:
    ...
    return v.All(
        v.Coerce(str),
        convert_not_found(
            lambda v: cloud_session.find_external_ip(v), "Not a valid external ip."
        ),
        ip_available,
        lambda ip: ip.id,
    )
...
def cloud_volume_constraint(cloud_session, options, **kwargs):
    ...
    return v.All(
        v.Coerce(str),
        convert_not_found(
            lambda v: cloud_session.find_volume(v), "Not a valid volume."
        ),
        min_size,
        lambda v: v.id,
    )
