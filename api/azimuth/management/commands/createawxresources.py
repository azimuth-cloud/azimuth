...
CAAS_CREDENTIAL_TYPES = [
    {
        ...
        "description": "Authenticate with an OpenStack cloud using a previously acquired token.",  # noqa: E501
        "kind": "cloud",
        "inputs": {
            "fields": [
                {
...
class Command(BaseCommand):
    ...
    def wait_for_awx(self, connection):
        ...
        while True:
            try:
                connection.api_get("/")
            except (rackit.ConnectionError, rackit.ApiError) as exc:  # noqa: F841
    ...
    def ensure_project(self, connection, organisation, project_spec):
        ...
        self.stdout.write(f"Waiting for project to become available...")  # noqa: F541
